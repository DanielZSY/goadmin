...
import (
	"errors"
	ownModel "goiris/admin/app/model"
	"goiris/common"
	"runtime"
...
func (a *Adapter) open() {
	...
	db.LogMode(common.G_DBConfig.Mysql.ShowSql)
	a.db = db
...
func (a *Adapter) createTable() {
	if a.db.HasTable(&ownModel.CasbinRule{}) {
		return
	}

	err := a.db.CreateTable(&ownModel.CasbinRule{}).Error
	if err != nil {
		panic(err)
	}
}
...
func (a *Adapter) dropTable() {
	err := a.db.DropTable(&ownModel.CasbinRule{}).Error
	if err != nil {
		panic(err)
	}
}

func loadPolicyLine(line ownModel.CasbinRule, model model.Model) {
	lineText := line.PType
	if line.V0 != "" {
		lineText += ", " + line.V0
	}
...
func (a *Adapter) LoadFilteredPolicy(model model.Model, filter interface{}) error {
	...
	if err := a.db.Scopes(a.filterQuery2(a.db, filterValue)).Find(&lines).Error; err != nil {
		return err
	}
...
// 由于不能同时过滤g和p的租户，所以只能改一下
// 形如：SELECT * FROM `casbin_rule`  WHERE (p_type in ('p','g')) AND (v1 in ('a') OR v2 in ('a'))
func (a *Adapter) filterQuery2(db *gorm.DB, filter Filter) func(db *gorm.DB) *gorm.DB {
...
func savePolicyLine(ptype string, rule []string) ownModel.CasbinRule {
	line := ownModel.CasbinRule{}
...
func (a *Adapter) RemoveFilteredPolicy(sec string, ptype string, fieldIndex int, fieldValues ...string) error {
	...
	if fieldIndex <= 6 && 6 < fieldIndex+len(fieldValues) {
		line.V5 = fieldValues[6-fieldIndex]
	}
	err := rawDelete(a.db, line)
	return err
}

func rawDelete(db *gorm.DB, line ownModel.CasbinRule) error {
	...
	err := db.Delete(ownModel.CasbinRule{}, args...).Error
	return err
}
